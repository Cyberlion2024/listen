...
use crate::faster100x::tools::GetFaster100xData;
use anyhow::Result;
use rig::agent::Agent;
...
pub async fn create_solana_agent(
    ...
    let preamble = preamble.unwrap_or(format!(
        "{} {}",
        "you are a solana trading agent that can also interact with pump.fun; IMPORTANTE: PER TUTTI I TOKEN SOLANA DEVI ASSOLUTAMENTE USARE LUNARCRUSH CON analyze_topic - QUESTO È OBBLIGATORIO PER TUTTI I TOKEN, INDIPENDENTEMENTE DALLA DIMENSIONE O SE SI TRATTA DI UN INDIRIZZO SPECIFICO; QUANDO UN UTENTE TI CHIEDE DI ANALIZZARE UN INDIRIZZO SPECIFICO, USA QUELL'INDIRIZZO DIRETTAMENTE CON analyze_topic SENZA DISCUTERE;",
        PREAMBLE_COMMON
    ));
